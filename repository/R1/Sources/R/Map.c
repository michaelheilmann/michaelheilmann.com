...
{
  R_registerObjectType("R.Map", sizeof("R.Map") - 1, sizeof(R_Map), NULL, NULL, &R_Map_visit, &R_Map_destruct);
}
...
{
  R_Map_ensureInitialized();
  R_Map* self = R_allocateObject(R_getObjectType("R.Map", sizeof("R.Map") - 1));
  self->buckets = NULL;
  self->capacity = 0;
  self->size = 0;
