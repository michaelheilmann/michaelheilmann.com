...
Rex_defineObjectType(u8"R.Interpreter.Class", R_Interpreter_Class, u8"Arcadia.Object", R_Object, &_typeOperations);
...
{
  R_Value key = { .tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = method->unqualifiedName };
  R_Value value = R_Map_get(process, self->classMembers, key);
  if (!Arcadia_Value_isVoidValue(&value)) {
    Arcadia_Process_setStatus(process, Arcadia_Status_Exists);
    Arcadia_Process_jump(process);
...
{
  R_Value key = { .tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = variable->name };
  R_Value value = R_Map_get(process, self->classMembers, key);
  if (!Arcadia_Value_isVoidValue(&value)) {
    Arcadia_Process_setStatus(process, Arcadia_Status_Exists);
    Arcadia_Process_jump(process);
...
{ 
  ...
  if (self->extendedClassName) {
    ...
    do {
      R_Value k = { .tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = current->className };
      R_Value v = { .tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = self };
...
{ 
  ...
  for (Arcadia_SizeValue i = 0, n = R_List_getSize(members); i < n; ++i) {
    ...
    if (Arcadia_Type_isSubType(Arcadia_Value_getType(process, &v), _R_Interpreter_Method_getType(process))) {
      ...
      R_Value k2 = { .tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = m->unqualifiedName };
