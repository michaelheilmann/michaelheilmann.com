...
{
  Arcadia_Arrays_resizeByFreeCapacity(Arcadia_Process_getThread(process), Arms_getDefaultMemoryManager(), &self->p, sizeof(Arcadia_Natural8Value), self->sz, &self->cp, numberOfBytes, Arcadia_Arrays_ResizeStrategy_Type4);
  Arcadia_Process_copyMemory(process, self->p + self->sz, bytes, numberOfBytes);
