  ...
  )
{
  ...
  Arcadia_Integer64Value e = s->exponent + s->shift;
  // THEOREM
  ...
  if (s->significand == Arcadia_Natural64Value_Literal(0) || e < Arcadia_Integer64Value_Literal(-342)) {
  ...
  Arcadia_SizeValue numberOfLeadingZeroes = Arcadia_countLeadingZeroesNatural64Value(thread, s->significand);
  Arcadia_Natural64Value significand = s->significand << numberOfLeadingZeroes;

  Arcadia_Natural64Value upper, lower;
  Arcadia_computeProductApproximation(thread, significand, s->exponent + s->shift, &upper, &lower);
...
#define MAX_EXPONENT_ROUND_TO_EVEN (23)
  ...
  answer.exponent = Arcadia_power(s->exponent + s->shift) + upperBit - numberOfLeadingZeroes - (Arcadia_Real64Value_MinimalExponent - 1);
  ...
  if (lower <= 1 && MIN_EXPONENT_ROUND_TO_EVEN <= s->exponent + s->shift && s->exponent + s->shift <= MAX_EXPONENT_ROUND_TO_EVEN && ((answer.significand & 3) == 1)) {
