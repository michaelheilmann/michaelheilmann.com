...
#define LEMIRE_LARGEST_POWER 308
  ...
  if (!(LEMIRE_SMALLEST_POWER <= exponent && exponent <= LEMIRE_LARGEST_POWER)) {
    return Arcadia_BooleanValue_False;
  }
  answer.negative = negative;
  if (significand == Arcadia_Natural64Value_Literal(0) || exponent < Arcadia_Integer64Value_Literal(-342)) {
    answer.significand = 0;
    answer.exponent = 0;
    *result = answer;
    return Arcadia_BooleanValue_True;
  }
  if (exponent > Arcadia_Integer64Value_Literal(+308)) {
    answer.significand = 0;
  ...
  Arcadia_SizeValue numberOfLeadingZeroes = Arcadia_countLeadingZeroesNatural64Value(thread, significand);
  significand <<= numberOfLeadingZeroes;

  Arcadia_Natural64Value upper, lower;
  Arcadia_computeProductApproximation(thread, significand, exponent, &upper, &lower);
...
#define MAX_EXPONENT_ROUND_TO_EVEN (23)
  ...
  answer.exponent = Arcadia_power(exponent) + upperBit - numberOfLeadingZeroes - (Arcadia_Real64Value_MinimalExponent - 1);
  ...
  if (lower <= 1 && MIN_EXPONENT_ROUND_TO_EVEN <= exponent && exponent <= MAX_EXPONENT_ROUND_TO_EVEN && ((answer.significand & 3) == 1)) {
