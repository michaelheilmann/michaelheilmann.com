...
#define MaximalDecimalSignificandDigits_Lemire 19  
  ...
  Arcadia_Integer64Value exponentInteger64 = Arcadia_BigInteger_toInteger64(thread, s.exponent);



  /* ~~ Clinger's method ~~ */
  if (totalSignificandDigits <= MaximalDecimalSignificandDigits_Clinger && MinimalExponent_Clinger <= exponentInteger64 && exponentInteger64 <= MaximalExponent_Clinger) {
    ...
    Arcadia_Natural64Value significandNatural64 = Arcadia_BigInteger_toNatural64(thread, s.significand);
    Arcadia_Real64Value v = (Arcadia_Real64Value)significandNatural64;
    if (exponentInteger64 >= 0) {
      v *= PowersOfTenReal64Value[+exponentInteger64];
    } else {
      v /= PowersOfTenReal64Value[-exponentInteger64];
    }
  ...
  if (totalSignificandDigits <= MaximalDecimalSignificandDigits_Lemire) {
    ...
    Arcadia_Natural64Value significandNatural64 = Arcadia_BigInteger_toNatural64(thread, s.significand);
    Arcadia_ToReal64_Result result;
    if (Arcadia_lemireReal64(thread, &result, significandNatural64, exponentInteger64, s.negative)) {
      Arcadia_Real64Value v;
      Arcadia_ToReal64_BitsToValue(thread, &v, &result);
      return v;
    } else {
