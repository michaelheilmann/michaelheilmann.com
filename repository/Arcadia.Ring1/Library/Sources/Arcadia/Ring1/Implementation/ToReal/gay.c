  ...
  )
{
  ...
  k = source->exponent.integral.start + source->exponent.integral.leadingZeroes.length;// + source->exponent.trailingZeroes.length;
...
static void to_extended(Arcadia_Thread* thread, Arcadia_ToReal64_Result* result, Arcadia_Real64Value value) {
  ...
  int32_t bias = Arcadia_Real64Value_NumberOfExplicitSignificandBits -
                 Arcadia_Real64Value_MinimalExponent;
  uint64_t bits;
  ...
  )
{
  ...
  Arcadia_Natural32Value pow5Exp = (Arcadia_Natural32Value)-pow2Exp;
  ...
  )
{
  ...
  Arcadia_Integer32Value exponent = numberLiteralApproximation.scientific.scientificExponent + 1 - numberLiteralApproximation.scientific.digitsConsidered;
  {
    ...
    exponent += numberLiteralApproximation.exponent;
  }
