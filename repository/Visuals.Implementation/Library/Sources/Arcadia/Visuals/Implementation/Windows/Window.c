  ...
  )
{
  ...
  _self->system = Arcadia_ArgumentsValidation_getObjectReferenceValue(thread, &argumentValues[0], _Arcadia_Visuals_Windows_System_getType(thread));
...
Arcadia_Visuals_Windows_Window_create
  (
    Arcadia_Thread* thread,
    Arcadia_Visuals_Windows_System* systme,
    Arcadia_Visuals_Windows_DisplayDevice* displayDevice
  )
{
  Arcadia_Value argumentValues[] = {
    systme ? Arcadia_Value_makeObjectReferenceValue(systme) : Arcadia_Value_makeVoidValue(Arcadia_VoidValue_Void),
