  ...
  )
{
  ...
  if (R_ByteBuffer_isEqualTo_pn(process, context->context->temporaryBuffer, "include", sizeof("include") - 1)) {
    ...
    Arcadia_Value_setObjectReferenceValue(&t, (Arcadia_ObjectReferenceValue)R_String_create(process, t));
    t = R_Map_get(process, context->environment, t);
    if (!Arcadia_Value_isObjectReferenceValue(&t)) {
      Arcadia_Process_setStatus(process, Arcadia_Status_ArgumentTypeInvalid);
      Arcadia_Process_jump(process);
    }
    R_Object* object = Arcadia_Value_getObjectReferenceValue(&t);
...
Rex_defineObjectType(u8"Tools.TemplateEngine.FileContext", FileContext, u8"Arcadia.Object", R_Object, &_typeOperations);
...
{
  R_Value argumentValues[] = { {.tag = Arcadia_ValueTag_ObjectReference, .objectReferenceValue = (Arcadia_ObjectReferenceValue)context },
