...
static const Arcadia_ObjectType_Operations _Arcadia_Engine_Demo_Scene_objectTypeOperations = {
  ...
  .construct = (Arcadia_Object_ConstructCallbackFunction*)&Arcadia_Engine_Demo_Scene_construct,
  ...
  )
{
  ...
  self->updateAudials = NULL;
  self->updateLogics = NULL;
  self->updateVisuals = NULL;
...
{ self->updateAudials(thread, self, tick, width, height); }
...
{ self->updateLogics(thread, self, tick); }
...
{ self->updateVisuals(thread, self, tick, width, height); }
