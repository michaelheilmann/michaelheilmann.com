...
callback
  (
    Arcadia_WeakReference* context,
    void* object
  )
{
  Arcadia_Value_setVoidValue(&context->value, Arcadia_VoidValue_Void);
}
  ...
  if (0 == numberOfArgumentValues) {
    ...
    switch (Arcadia_Value_getTag(&_self->value)) {
      case Arcadia_ValueTag_Atom: {
        Arms_addNotifyDestroy(self->atomValue, _self, &callback);
      } break;
      ...
      case Arcadia_ValueTag_ImmutableByteArray: {
        Arms_addNotifyDestroy(self->immutableByteArrayValue, _self, &callback);
      } break;
  ...
  )
{
  if (value) {
    ...
    Arcadia_WeakReference* self = Arcadia_allocateObject(thread, _Arcadia_WeakReference_getType(thread), 0, &argumentValues[0]);
    return self;
  }
}
