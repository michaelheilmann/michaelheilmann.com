  ...
  )
{
  ...
  if (Arcadia_BigInteger_getBitLength(thread, powerOfTwo) > FLOAT64_POW5_INV_BITCOUNT + 1) {
    Arcadia_Thread_setStatus(thread, Arcadia_Status_ArgumentValueInvalid);
    Arcadia_Thread_jump(thread);
  }
  return powerOfTwo;
  ...
  )
{
  ...
  Arcadia_StringBuffer_append_pn(thread, stringBuffer, u8"static const Arcadia_Natural64Value ", sizeof(u8"static const Arcadia_Natural64Value "));
  ...
  )
{
  ...
  for (Arcadia_SizeValue i = 0; i < length; ++i) {
    ...
    for (Arcadia_SizeValue j = 0; j < 16; ++j) {
      Arcadia_SizeValue offset = i * 16 + j < Arcadia_List_getSize(thread, table) ? Arcadia_List_getInteger32ValueAt(thread, table, i * 16 + j) : 0;
      value |= offset << (j << 1);
    }
    if (i % entriesPerLine == 0) {
      Arcadia_StringBuffer_append_pn(thread, stringBuffer, u8"  ", sizeof(u8"  ") - 1);
  ...
  )
{
  ...
  Arcadia_StringBuffer_insertBack(thread, target, Arcadia_Value_makeImmutableUtf8StringValue(Arcadia_ImmutableUtf8String_createFromInteger32(thread, FLOAT64_POW5_INV_BITCOUNT)));
  ...
  Arcadia_StringBuffer_insertBack(thread, target, Arcadia_Value_makeImmutableUtf8StringValue(Arcadia_ImmutableUtf8String_createFromInteger32(thread, FLOAT64_POW5_BITCOUNT)));
  Arcadia_StringBuffer_append_pn(thread, target, u8"\n", sizeof(u8"\n") - 1);
  //
