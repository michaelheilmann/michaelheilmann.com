  ...
  )
{
  ...
  for (Arcadia_Natural32Value i = 0; i < FLOAT64_NEG_TABLE_SIZE; i++) {
    ...
    Arcadia_BigInteger* error = Arcadia_BigInteger_create(thread);
    Arcadia_BigInteger_copy(thread, error, invMultiplier(thread, i));
    Arcadia_BigInteger_subtract(thread, error, result);

    if (Arcadia_BigInteger_isNegative(thread, error) || Arcadia_BigInteger_compareTo(thread, error, TWO) > 0) {
      Arcadia_Thread_setStatus(thread, Arcadia_Status_ArgumentValueInvalid);
      Arcadia_Thread_jump(thread);
    }
    Arcadia_List_insertBack(thread, invErrorTable, Arcadia_Value_makeInteger32Value(Arcadia_BigInteger_toInteger32(thread, error)));
  }
