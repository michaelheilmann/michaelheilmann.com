  ...
  )
{
  Arcadia_Value argumentValues[] = {
    ...
    NULL != system ? Arcadia_Value_makeObjectReferenceValue(system) : Arcadia_Value_makeVoidValue(Arcadia_VoidValue_Void),
  };
  Arcadia_Visuals_ApplicationQuitRequestedEvent* self = Arcadia_allocateObject(thread, _Arcadia_Visuals_ApplicationQuitRequestedEvent_getType(thread), 2, &argumentValues[0]);
  return self;
}