...
static const Arcadia_ObjectType_Operations _objectTypeOperations = {
  ...
  .construct = (Arcadia_Object_ConstructorCallbackFunction*) & Arcadia_Imaging_Operations_Fill_constructImpl,
...
{
  if (NULL == self->definition->colorDefinition) {
    Arcadia_Thread_setStatus(thread, Arcadia_Status_OperationInvalid);
    Arcadia_Thread_jump(thread);
  }
  Arcadia_Natural8Value red = self->definition->colorDefinition->red;
  Arcadia_Natural8Value green = self->definition->colorDefinition->green;
  Arcadia_Natural8Value blue = self->definition->colorDefinition->blue;
